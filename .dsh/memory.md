...
- npm 发布 scoped 包的认证坑：账号开 2FA 时旧 bypass-2FA token 会被 npm 禁 direct publish（报 404/E403），须先 npm login 重登；publish 还要 EOTP 网页认证，必须真终端跑 npm publish --tag spike（agent 跑会因 auth URL 掩码 + 无浏览器卡住）。发布后 npm view 可能短暂 404（metadata CDN 缓存了发布前的 404），tarball 与 maintainer search 立即可见、几分钟自愈，别误判失败 #npm #dsh-reflect #publish #2fa
