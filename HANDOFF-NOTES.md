...
6. **实现进度（按设计文档 §4 切分）**：
   ...
   - **第 3 步骨架**（`0.0.1-spike.7`，64/64 测试）：`session/event` 探针监听器（`{global:true}`，写 events.jsonl，`DSH_REFLECT_EVENT_LOG=off` 关闭）+ `/reflect-distill` 命令（当前是 stub，等探针确认监听范围后接入 `listSessions` + `filterEvents`）。**待重启验**：重启后读 `~/.dsh/reflect/events.jsonl`，看 `sessionId` 分布——如果只有当前会话的 id，说明 `{global:true}` 不跨会话；如果有多条不同 sessionId，说明全局监听生效，可以开自动蒸馏回路。
