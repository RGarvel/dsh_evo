...
5. 发版决策：@garvel/dsh-reflect 首发用 --tag spike；发布必须你终端跑（EOTP 网页认证）。当前版本 `0.0.1-spike.4`，`files` 已含 `lib`/`docs`/`cordis.patch.yml`/`README`/`LICENSE`，`dsh.bundle` 声明也已就位——发出去的包可直接 `dsh plugin add @garvel/dsh-reflect`。
6. **实现进度（按设计文档 §4 切分）**：
   ...
   - **第 4 步 ✅ 已落地**（同日，`0.0.1-spike.4`，61/61 测试）：注入换成 `ctx.systemPrompt.section({name, order:950, text:(context)=>…})`（源码核对：`PromptSection.order` 必填、`text` 可传 provider、`section()` 自带 disposer、重名直接 throw）；工作区层随 `context.agent?.session?.header?.cwd` 一起注入；预算改 token 口径，但**除数直接取 harness 自己的 `estimateSystemTokens` = `ceil(len/4)+4`**——`dsh-token-meter` 只导出 `TokenMeter` 类，`estimateMessage` 面向会话消息（role framing + 每块 `+4`），拿伪造 Message 去调只会让预算与循环实际计费不一致，所以 4 chars/token 就是契约（`CHARS_PER_TOKEN` 在 store.js）；超限**整行**从尾部丢并声明"还有 N 条未注入"；缺 `agent` 时 `logger.warn` 一次不静默；队列只报条数、内容绝不进提示词（I2 守着）；env 增至四个（`DSH_REFLECT_INJECT_MAX_TOKENS` 默认 600，`_INJECT_MAX_CHARS` 保留为直接覆盖且优先级更高）；
   - **仍不自动**：没有生产者往队列里写东西；全局队列现有一条探针（`commands.register()`/`section()` 自管 dispose 那条），等你 `/reflect-review global approve 1`；
   - **第 4 步实机验证：新代码活着，但工作区层没出现（这一项未过）**。重启 #4 后本轮注入块里确有 spike.4 才有的队列提示与 `reflect_pending` 规约句，可内容是 `Global lessons` 6 条 + "还有 1 条未注入"（=7 条全局，工作区 3 条 entirely absent；若 cwd 拿到了，被裁的应当是 4 条）。同期 `reflect_recall(scope:"workspace", workspace_dir:"D:\dsh_evo")` 在**运行中的进程里**返回全部 3 条正文 → **读盘/写盘/编码/权限全部无罪**，断点只在 provider 里那条 `context.agent?.session?.header?.cwd`。源码侧证据互相矛盾：官方 `dsh-agent-loop:1026` 用同一表达式注册 `{{cwd}}` 变量、`assembleContextFor()` 返回 `{agent, scope:agent}`、全树只有 `dsh-agent-loop:497` 一处 `assemble()` 调用；但 `dsh-user-approval` 注释明说"bare assemble()（tests, diagnostics）没有 session"。推理不能定案 → **`spike.5` 装自证探针**：provider 每次组装把 `{stage, cwd, global, workspace, pending, chars, budgetChars, assemblies, at}` 覆盖写 `~/.dsh/reflect/assembly.json`，`stage` 直接点名断在 `no-agent`/`no-session`/`no-header`/`no-cwd`/`ok` 哪一格（`DSH_REFLECT_ASSEMBLY_FILE=off` 关闭；写失败绝不影响本轮）。
   - **免重启的旁证**（当前进程就能跑）：`/reflect-review` 不带参数走的是 `invocation.agent` 的同一条链——回"该会话没有工作区" ⇒ agent 上就没 cwd；能列出队列 ⇒ 断点在 assemble context 那一侧。
   - **同步纪律**（本轮踩到）：部署副本 `index.js` 与源码差 51 行（探针没同步）而 `store.js` 差 0 行——**只比对一个文件会漏**。核对固化为 `Compare-Object` 逐文件比 `lib/*.js`，且版本号与部署字节一一对应。
