...
6. **实现进度（按设计文档 §4 切分）**：
   ...
   - **验证状态**（spike.15 部署后）：待重启→本会话发 1 条→读 `distill-debug.log` 看是否出现 `streamed response`/`queued N`，成功则 `pending.md` 落 `@src:...@distill` 行＝端到端跑通。若 bail 在 `no LLM route` 则 provider/model 解析仍需调；若 bail 在 stream failed 则是网络/额度。哨兵文件 `~/.dsh/reflect/auto-distill.on` 在＝开关生效。
